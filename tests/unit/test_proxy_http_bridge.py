...
def test_http_bridge_eventless_precreated_deadline_uses_current_send_and_client_safe_cap() -> None:
    ...
    request_state.fresh_bridge_reattach_startup_timeout_seconds = 30.0
    assert (
        http_bridge_helpers_module._http_bridge_eventless_precreated_deadline(
            request_state,
            stuck_gate_retire_after_seconds=300.0,
    ...
    request_state.fresh_bridge_reattach_startup_timeout_seconds = None
...
@pytest.mark.asyncio
async def test_http_bridge_receive_timeout_preserves_original_request_deadline() -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    now = time.monotonic()
    request_state = _make_eventless_http_bridge_owner(sent_at=now)
    request_state.started_at = now
    request_state.bridge_request_deadline = now + 0.05
    ...
    assert 0.0 < timeout.timeout_seconds <= 0.05
    assert timeout.error_code == "upstream_request_timeout"


@pytest.mark.asyncio
async def test_http_bridge_cancel_safe_cleanup_preserves_caller_cancellation() -> None:
    ...
    task = asyncio.create_task(http_bridge_streaming_module._await_http_bridge_cancel_safe_cleanup(cleanup()))
...
@pytest.mark.asyncio
async def test_http_bridge_cancel_safe_cleanup_bounds_blocked_cleanup(
    ...
    monkeypatch.setattr(
        ...
        "_HTTP_BRIDGE_CANCEL_SAFE_CLEANUP_TIMEOUT_SECONDS",
        0.01,
    )
    await http_bridge_streaming_module._await_http_bridge_cancel_safe_cleanup(cleanup())
    await asyncio.sleep(0)

    assert cleanup_cancelled.is_set()
    assert not http_bridge_streaming_module._HTTP_BRIDGE_CANCEL_SAFE_CLEANUP_TASKS


@pytest.mark.asyncio
async def test_http_bridge_cancel_safe_cleanup_keeps_cancellation_when_cleanup_fails() -> None:
    cleanup_started = asyncio.Event()
    release_cleanup = asyncio.Event()
...
async def test_stale_response_rejection_preserves_newer_same_session_registration(
    ...
    try:
        ...
        assert "resp-race" in session.durable_previous_response_ids
...
@pytest.mark.asyncio
async def test_previous_response_persistence_failure_keeps_only_local_alias(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session()
    session.durable_session_id = "durable-local-fallback"
    session.durable_owner_epoch = 1
    service._http_bridge_sessions[session.key] = session
    service._durable_bridge = SimpleNamespace(
        register_previous_response_id=AsyncMock(side_effect=RuntimeError("database unavailable"))
    )
    monkeypatch.setattr(http_bridge_helpers_module, "get_settings", _make_app_settings)

    registered = await service._register_http_bridge_previous_response_id(session, "resp-local-fallback")
    ...
    assert "resp-local-fallback" in session.previous_response_ids
    assert "resp-local-fallback" not in session.durable_previous_response_ids
    assert service._http_bridge_previous_response_index[alias_key] == session.key
...
@pytest.mark.asyncio
async def test_completed_unanchored_forks_release_stream_capacity_beyond_cap(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    ...
    async def register_previous_response_id(
        ...
        **_kwargs: Any,
        ...
        session.durable_previous_response_ids.add(response_id)
        return True

    register_previous = AsyncMock(side_effect=register_previous_response_id)
    finalize = AsyncMock()
    monkeypatch.setattr(service, "_register_http_bridge_previous_response_id", register_previous)
    monkeypatch.setattr(service, "_finalize_websocket_request_state", finalize)
    caps = AccountConcurrencyCaps(response_create_limit=4, stream_limit=2)
    ...
    for index in range(5):
        ...
        request_state = proxy_service._WebSocketRequestState(
            request_id=f"req-completed-fork-{index}",
            model="gpt-5.2",
            service_tier=None,
            reasoning_effort=None,
            api_key_reservation=None,
            ...
            json.dumps(
                {
                    ...
                    "response": {
                        "id": f"resp_completed_fork_{index}",
                        "object": "response",
                        "status": "in_progress",
                        "output": [],
                    },
    ...
    replacement = await service._load_balancer.acquire_account_lease(
        "acc-bridge",
        kind="stream",
        concurrency_caps=caps,
    )
    assert replacement is not None
    await service._load_balancer.release_account_lease(replacement)
    assert register_previous.await_count == 5
    assert finalize.await_count == 5


@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("blocker", "expected_marked"),
    [
        ("pending", False),
        ("queued", False),
        ("admission", True),
...
async def test_completed_unanchored_fork_waits_for_quiescence(
    monkeypatch: pytest.MonkeyPatch,
    blocker: str,
    ...
    if blocker == "pending":
        ...
        session.unanchored_reservation_id = "scope-still-reserved"
    close = AsyncMock()
    monkeypatch.setattr(service, "_close_http_bridge_session_bounded", close)
    response_id = f"resp-blocked-{blocker}"
    session.durable_previous_response_ids.add(response_id)

    marked = await service._mark_completed_unanchored_http_bridge_fork_for_retirement(
        session,
        response_id=response_id,
    )
    retired = await service._retire_http_bridge_after_drain_if_ready(session)

    assert marked is expected_marked
    assert session.completed_unanchored_fork_retirement_candidate is True
    assert retired is False
    assert session.upstream_control.retire_after_drain is expected_marked
    close.assert_not_awaited()


@pytest.mark.asyncio
async def test_completed_unanchored_fork_retires_after_pending_sibling_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "completed-before-sibling-failure",
            None,
        )
    )
    sibling = proxy_service._WebSocketRequestState(
        request_id="req-failing-sibling",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session.queued_request_count = 1
    response_id = "resp-completed-before-sibling-failure"
    session.previous_response_ids.add(response_id)
    session.durable_previous_response_ids.add(response_id)
    close = AsyncMock()
    monkeypatch.setattr(service, "_close_http_bridge_session_bounded", close)

    assert (
        await service._mark_completed_unanchored_http_bridge_fork_for_retirement(
            session,
            response_id=response_id,
        )
        is False
    ...
    assert await service._retire_http_bridge_after_drain_if_ready(session) is True
    close.assert_awaited_once_with(session, reason="retire_after_drain")


@pytest.mark.asyncio
@pytest.mark.parametrize("blocker", ["admission", "handoff"])
async def test_retire_after_drain_waits_for_late_admission_ownership(
    monkeypatch: pytest.MonkeyPatch,
    blocker: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    ...
    if blocker == "admission":
        session.admission_waiter_count = 1
    else:
        session.unanchored_reservation_id = "scope-late-handoff"
    close = AsyncMock()
    monkeypatch.setattr(service, "_close_http_bridge_session_bounded", close)
...
@pytest.mark.asyncio
async def test_http_bridge_reader_rechecks_retirement_after_clearing_early_wakeup(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    receive = AsyncMock(side_effect=AssertionError("retiring reader must not start another receive"))
    session = _make_bridge_session(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "early-retirement-wakeup",
            None,
        )
    )
    session.upstream = cast(
...
@pytest.mark.parametrize(
    "key",
    [
        proxy_service._HTTPBridgeSessionKey("session_header", "canonical-session", None),
        _make_account_neutral_replay_session_key("recovery-lane"),
    ],
    ids=["canonical-session", "account-neutral-recovery"],
)
async def test_completed_fork_retirement_excludes_reusable_lane_kinds(
    monkeypatch: pytest.MonkeyPatch,
    key: proxy_service._HTTPBridgeSessionKey,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    ...
    close = AsyncMock()
    monkeypatch.setattr(service, "_close_http_bridge_session_bounded", close)
    session.durable_previous_response_ids.add("resp-excluded-lane")

    marked = await service._mark_completed_unanchored_http_bridge_fork_for_retirement(
        session,
        response_id="resp-excluded-lane",
    )
...
@pytest.mark.asyncio
async def test_completed_unanchored_fork_keeps_local_fallback_when_alias_is_not_durable(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-local-alias-fallback",
        response_id="resp_local_alias_fallback",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
...
@pytest.mark.parametrize(
    "durable_response_id",
    ["resp_first_completed", "resp_second_completed"],
    ids=["last-local-only", "first-local-only"],
)
async def test_completed_unanchored_fork_with_sibling_keeps_local_fallback(
    monkeypatch: pytest.MonkeyPatch,
    durable_response_id: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    completed_state = proxy_service._WebSocketRequestState(
        request_id="req-first-completed",
        response_id="resp_first_completed",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    failed_state = proxy_service._WebSocketRequestState(
        request_id="req-second-failed",
        response_id="resp_second_completed",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session = _make_bridge_session(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "fork-drains-after-error",
            None,
        ),
        pending_requests=deque([completed_state, failed_state]),
        queued_request_count=2,
    )
    service._http_bridge_sessions[session.key] = session
    ...
    async def register_previous_response_id(
        ...
        **_kwargs: Any,
        ...
        target.previous_response_ids.add(response_id)
        if response_id == durable_response_id:
            target.durable_previous_response_ids.add(response_id)
        return True

    monkeypatch.setattr(
        service,
        "_register_http_bridge_previous_response_id",
        AsyncMock(side_effect=register_previous_response_id),
    )
    monkeypatch.setattr(service, "_finalize_websocket_request_state", AsyncMock())
    ...
    assert session.closed is False
    assert session.upstream_control.retire_after_drain is False
    assert list(session.pending_requests) == [failed_state]
    assert await service._retire_http_bridge_after_drain_if_ready(session) is False
    ...
    assert not session.pending_requests
    assert session.queued_request_count == 0
    local_only_response_id = (
        "resp_second_completed" if durable_response_id == "resp_first_completed" else "resp_first_completed"
    )
    assert local_only_response_id in session.previous_response_ids
    assert local_only_response_id not in session.durable_previous_response_ids
    cast(Any, session.upstream).close.assert_not_awaited()


@pytest.mark.asyncio
async def test_retiring_fork_rechecks_after_late_admission(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(
    ...
    async def acquire_then_mark_retiring(*args: Any, **kwargs: Any) -> None:
        await original_acquire(*args, **kwargs)
        session.upstream_control.reconnect_requested = True
        session.upstream_control.retire_after_drain = True

    monkeypatch.setattr(service, "_acquire_request_state_response_create_admission", acquire_then_mark_retiring)
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-late-admission-race",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    with pytest.raises(ProxyResponseError) as exc_info:
        await service._submit_http_bridge_request(
            session,
            request_state=request_state,
            text_data='{"type":"response.create","model":"gpt-5.2","input":"hello"}',
            queue_limit=4,
        )
...
@pytest.mark.asyncio
async def test_retiring_fork_retries_after_releasing_handoff_reservation() -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "fork-handoff-release",
            None,
        )
    )
    session.upstream_control.reconnect_requested = True
    session.upstream_control.retire_after_drain = True
    session.unanchored_reservation_id = "scope-handoff-release"
    service._http_bridge_sessions[session.key] = session
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-handoff-release",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    request_scope_token = set_request_scope_id("scope-handoff-release")
    try:
        with pytest.raises(ProxyResponseError) as exc_info:
            await service._submit_http_bridge_request(
                session,
...
@pytest.mark.asyncio
async def test_handoff_release_wakes_live_reader_without_scheduling_close(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "fork-reader-owned-retirement",
            None,
        )
    )
    session.upstream_control.reconnect_requested = True
    session.upstream_control.retire_after_drain = True
    session.unanchored_reservation_id = "scope-reader-owned"
    reader_release = asyncio.Event()

    async def wait_for_reader_release() -> None:
        await reader_release.wait()

    reader = asyncio.create_task(wait_for_reader_release())
    ...
    try:
        released = http_bridge_helpers_module._release_http_bridge_unanchored_handoff(
            session,
            request_scope_id="scope-reader-owned",
            service=service,
        )
...
@pytest.mark.asyncio
async def test_submit_unwind_does_not_steal_retirement_without_handoff(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "fork-normal-submit-unwind",
            None,
        )
    )
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-normal-submit-unwind",
        model="gpt-5.2",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    async def complete_during_submit(*_args: Any, **_kwargs: Any) -> None:
        session.upstream_control.reconnect_requested = True
        session.upstream_control.retire_after_drain = True

    retire = AsyncMock(return_value=True)
    monkeypatch.setattr(service, "_submit_http_bridge_request_with_handoff", complete_during_submit)
...
async def test_reconnect_http_bridge_session_passes_dashboard_reset_window_to_selection(
    ...
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key=proxy_service._HTTPBridgeSessionKey("prompt_cache", "soft-reconnect", None))
    ...
    async def select_account(_deadline: float, **kwargs: object) -> proxy_service.AccountSelection:
        selection_deadlines.append(_deadline)
        selection_kwargs.append(kwargs)
        return proxy_service.AccountSelection(account=None, error_message="No active accounts available")
    ...
    request_state = proxy_service._WebSocketRequestState(
        ...
        bridge_request_deadline=time.monotonic() + 5.0,
    )
    monkeypatch.setattr(proxy_service, "get_settings", lambda: _make_app_settings())
    monkeypatch.setattr(
        proxy_service, "get_settings_cache", lambda: SimpleNamespace(get=AsyncMock(return_value=settings))
    ...
    assert selection_deadlines == [request_state.bridge_request_deadline]
    assert request_state.response_create_sent_at == 1.0
...
async def test_stream_via_http_bridge_clears_injected_anchor_after_owner_unavailable_fresh_resend(
    ...
    has_live_local_session = AsyncMock(return_value=False)
    monkeypatch.setattr(service, "_http_bridge_has_live_local_session", has_live_local_session)
    monkeypatch.setattr(
        service,
...
async def test_process_http_bridge_upstream_text_masks_unmatched_missing_tool_output_followups(
    ...
    session = proxy_service._HTTPBridgeSession(
        key=proxy_service._HTTPBridgeSessionKey(
            "internal_unanchored_parallel",
            "completed-before-grouped-failure",
            None,
        ),
        headers={"x-codex-session-id": "sid-123"},
        affinity=proxy_service._AffinityPolicy(
...
@pytest.mark.asyncio
async def test_retry_http_bridge_request_rechecks_downstream_detach_before_resend(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    send_text = AsyncMock()
    session = _make_bridge_session(key_value="detach-during-fresh-retry")
    session.upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(send_text=send_text, close=AsyncMock()),
    )
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-detach-during-fresh-retry",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=1.0,
        previous_response_id="resp-before-detach",
        fresh_upstream_request_text='{"type":"response.create","input":"verified-full-resend"}',
        fresh_upstream_request_is_retry_safe=True,
        event_queue=asyncio.Queue(),
        transport="http",
    )
    ...
    recovered = await service._retry_http_bridge_request_on_fresh_upstream(
        session=session,
        request_state=request_state,
        text_data='{"type":"response.create","previous_response_id":"resp-before-detach"}',
        send_request=True,
    )
...
@pytest.mark.asyncio
async def test_downstream_detach_publishes_barrier_while_retry_send_is_blocked(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    send_started = asyncio.Event()
    ...
    session = _make_bridge_session(key_value="detach-during-blocked-retry-send")
    session.upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(send_text=send_text, close=AsyncMock()),
    )
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-detach-during-blocked-retry-send",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=1.0,
        previous_response_id="resp-before-blocked-send",
        fresh_upstream_request_text='{"type":"response.create","input":"verified-full-resend"}',
        fresh_upstream_request_is_retry_safe=True,
        event_queue=asyncio.Queue(),
        transport="http",
    ...
    retry_task = asyncio.create_task(
        service._retry_http_bridge_request_on_fresh_upstream(
            session=session,
            request_state=request_state,
            text_data='{"type":"response.create","previous_response_id":"resp-before-blocked-send"}',
            send_request=True,
        )
    )
    ...
    release_send.set()
    assert await retry_task is False


@pytest.mark.asyncio
async def test_retry_http_bridge_request_on_fresh_upstream_replays_verified_client_anchor_without_anchor(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    send_text = AsyncMock()
    session = _make_bridge_session(key_value="client-anchor-safe")
    session.upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(send_text=send_text, close=AsyncMock()),
    )
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-client-anchor-safe",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=1.0,
        previous_response_id="resp_client_safe",
        proxy_injected_previous_response_id=False,
        fresh_upstream_request_text='{"type":"response.create","input":"verified-full-resend"}',
        fresh_upstream_request_is_retry_safe=True,
        fresh_bridge_reattach_startup_timeout_seconds=30.0,
        transport="http",
    )
    reconnect = AsyncMock()
    monkeypatch.setattr(service, "_reconnect_http_bridge_session", reconnect)
    ...
    recovered = await service._retry_http_bridge_request_on_fresh_upstream(
        session=session,
        request_state=request_state,
        text_data='{"type":"response.create","previous_response_id":"resp_client_safe"}',
        send_request=True,
        require_same_account=True,
    )
    ...
    send_text_await = send_text.await_args
    assert send_text_await is not None
    assert json.loads(send_text_await.args[0])["input"] == "verified-full-resend"

    repeated = await service._retry_http_bridge_request_on_fresh_upstream(
        session=session,
        request_state=request_state,
        text_data=request_state.request_text or "{}",
        send_request=True,
...
async def test_stream_via_http_bridge_projects_plaintext_durable_full_resend_when_owner_is_unavailable(
    ...
    get_or_create = AsyncMock(
        side_effect=(
            ...
            if preprojected_fresh_reattach and replace_retired_gate
            else [capacity_unavailable, session]
            if preprojected_fresh_reattach
            else [owner_unavailable, session, replacement_session]
    ...
    if unsafe_replay_input is not None and unsafe_replay_input != "missing_owner":
        with pytest.raises(ProxyResponseError) as exc_info:
    ...
    if preprojected_fresh_reattach:
        ...
        assert first_call.kwargs["preferred_account_id"] is None
        assert first_call.kwargs["durable_lookup"] is None
        assert is_http_bridge_account_neutral_replay(
            kind=first_call.args[0].affinity_kind,
            key=first_call.args[0].affinity_key,
        )
        assert first_call.args[0] == second_call.args[0]
        assert first_call.kwargs["affinity"] == proxy_service._AffinityPolicy()
        assert first_call.kwargs["session_header_fallback_key"] is None
        assert first_call.kwargs["exclude_account_ids"] is None
        assert first_call.kwargs["allow_forward_to_owner"] is False
        assert all(key.lower() != "x-codex-turn-state" for key in first_call.kwargs["headers"])
        ...
        assert second_call.kwargs["exclude_account_ids"] is None
...
async def test_http_bridge_reader_wakes_and_retires_lone_eventless_owner_without_keepalives(
    ...
    if fresh_retry_fails:
        owner.previous_response_id = "resp-eventless-owner"
        owner.fresh_upstream_request_text = (
            '{"type":"response.create","model":"gpt-5.6-sol","input":"verified full resend"}'
        )
        owner.fresh_upstream_request_is_retry_safe = True
        owner.fresh_bridge_reattach_startup_timeout_seconds = 0.02
...
@pytest.mark.asyncio
@pytest.mark.parametrize("fresh_retry_armed", [False, True], ids=["ordinary", "fresh-retry-armed"])
async def test_http_bridge_eventless_timeout_yields_to_locked_send_failure_cleanup(
    monkeypatch: pytest.MonkeyPatch,
    fresh_retry_armed: bool,
) -> None:
    ...
    if fresh_retry_armed:
        owner.previous_response_id = "resp-send-failure-race"
        owner.request_text = '{"type":"response.create","previous_response_id":"resp-send-failure-race"}'
        owner.fresh_upstream_request_text = '{"type":"response.create","input":"full resend"}'
        owner.fresh_upstream_request_is_retry_safe = True
        owner.fresh_bridge_reattach_startup_timeout_seconds = 0.01
    async with session.pending_lock:
